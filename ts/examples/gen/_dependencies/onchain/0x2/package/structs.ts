...
export function isPublisher(type: string): boolean { type = compressSuiType(type); return type === `${PKG_V36}::package::Publisher`; }
...
export function isUpgradeCap(type: string): boolean { type = compressSuiType(type); return type === `${PKG_V36}::package::UpgradeCap`; }
...
export function isUpgradeTicket(type: string): boolean { type = compressSuiType(type); return type === `${PKG_V36}::package::UpgradeTicket`; }
...
export function isUpgradeReceipt(type: string): boolean { type = compressSuiType(type); return type === `${PKG_V36}::package::UpgradeReceipt`; }
...
export class UpgradeReceipt implements StructClass { __StructClass = true as const;
 ...
 static readonly $typeName = `${PKG_V36}::package::UpgradeReceipt`; static readonly $numTypeParams = 0; static readonly $isPhantom = [] as const;

 readonly $typeName = UpgradeReceipt.$typeName; readonly $fullTypeName: `${typeof PKG_V36}::package::UpgradeReceipt`; readonly $typeArgs: []; readonly $isPhantom = UpgradeReceipt.$isPhantom;

 readonly cap: ToField<ID>; readonly package: ToField<ID>

 private constructor(typeArgs: [], fields: UpgradeReceiptFields, ) { this.$fullTypeName = composeSuiType( UpgradeReceipt.$typeName, ...typeArgs ) as `${typeof PKG_V36}::package::UpgradeReceipt`; this.$typeArgs = typeArgs;
