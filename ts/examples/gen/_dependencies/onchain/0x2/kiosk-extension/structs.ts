...
export function isExtension(type: string): boolean { type = compressSuiType(type); return type === `${PKG_V36}::kiosk_extension::Extension`; }
...
export function isExtensionKey(type: string): boolean { type = compressSuiType(type); return type.startsWith(`${PKG_V36}::kiosk_extension::ExtensionKey` + '<'); }
...
export class ExtensionKey<T0 extends PhantomTypeArgument> implements StructClass { __StructClass = true as const;
 ...
 static fromFields<T0 extends PhantomReified<PhantomTypeArgument>>( typeArg: T0, fields: Record<string, any> ): ExtensionKey<ToPhantomTypeArgument<T0>> { return ExtensionKey.reified( typeArg, ).new( { dummyField: decodeFromFields("bool", fields.dummyField) } ) }
