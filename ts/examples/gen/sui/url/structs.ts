...
export function isUrl(type: string): boolean { type = compressSuiType(type); return type === `${PKG_V36}::url::Url`; }
...
export class Url implements StructClass { __StructClass = true as const;
 ...
 static readonly $typeName = `${PKG_V36}::url::Url`; static readonly $numTypeParams = 0; static readonly $isPhantom = [] as const;

 readonly $typeName = Url.$typeName; readonly $fullTypeName: `${typeof PKG_V36}::url::Url`; readonly $typeArgs: []; readonly $isPhantom = Url.$isPhantom;

 readonly url: ToField<String>

 private constructor(typeArgs: [], fields: UrlFields, ) { this.$fullTypeName = composeSuiType( Url.$typeName, ...typeArgs ) as `${typeof PKG_V36}::url::Url`; this.$typeArgs = typeArgs;

 this.url = fields.url; }

 static reified( ): UrlReified { return { typeName: Url.$typeName, fullTypeName: composeSuiType( Url.$typeName, ...[] ) as `${typeof PKG_V36}::url::Url`, typeArgs: [ ] as [], isPhantom: Url.$isPhantom, reifiedTypeArgs: [], fromFields: (fields: Record<string, any>) => Url.fromFields( fields, ), fromFieldsWithTypes: (item: FieldsWithTypes) => Url.fromFieldsWithTypes( item, ), fromBcs: (data: Uint8Array) => Url.fromBcs( data, ), bcs: Url.bcs, fromJSONField: (field: any) => Url.fromJSONField( field, ), fromJSON: (json: Record<string, any>) => Url.fromJSON( json, ), fromSuiParsedData: (content: SuiParsedData) => Url.fromSuiParsedData( content, ), fromSuiObjectData: (content: SuiObjectData) => Url.fromSuiObjectData( content, ), fetch: async (client: SuiClient, id: string) => Url.fetch( client, id, ), new: ( fields: UrlFields, ) => { return new Url( [], fields ) }, kind: "StructClassReified", } }
