...
export function isLinkedTable(type: string): boolean { type = compressSuiType(type); return type.startsWith(`${PKG_V36}::linked_table::LinkedTable` + '<'); }
...
export function isNode(type: string): boolean { type = compressSuiType(type); return type.startsWith(`${PKG_V36}::linked_table::Node` + '<'); }
