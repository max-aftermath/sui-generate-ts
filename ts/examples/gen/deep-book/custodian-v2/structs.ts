...
export function isAccount(type: string): boolean { type = compressSuiType(type); return type.startsWith(`${PKG_V21}::custodian_v2::Account` + '<'); }
...
export function isAccountCap(type: string): boolean { type = compressSuiType(type); return type === `${PKG_V21}::custodian_v2::AccountCap`; }
...
export function isCustodian(type: string): boolean { type = compressSuiType(type); return type.startsWith(`${PKG_V21}::custodian_v2::Custodian` + '<'); }
...
export class Custodian<T extends PhantomTypeArgument> implements StructClass { __StructClass = true as const;
 ...
 return Custodian.reified( typeArg, ).new( { id: decodeFromFieldsWithTypes(UID.reified(), item.fields.id), accountBalances: decodeFromFieldsWithTypes(Table.reified(reified.phantom("address"), reified.phantom(Account.reified(typeArg))), item.fields.accountBalances) } ) }
