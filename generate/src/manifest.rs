...
pub fn parse_gen_manifest_from_file(path: &Path) -> Result<GenManifest> {
    ...
    .with_context(|| format!("Unable to find check manifest at {:?}", path))?;
...
pub fn parse_gen_manifest_string(manifest_string: String) -> Result<TV> {
    toml::from_str::<TV>(&manifest_string).context("Unable to parse check manifest")
}
...
pub fn parse_gen_manifest(tval: TV) -> Result<GenManifest> {
    match tval {
        ...
        x => {
            bail!("Malformed check manifest {}. Expected a table at top level, but encountered a {}", x, x.type_str())
        }
    }
}
